...
#define CSM_FIELD_COUNT_MAX 65535
#define CSM_ARG_SEP_CHAR '/'
#define CSM_FIELD_SEP_CHAR '/'
...
static int object_post_fieldc(csm_bc_object *o);
...
static int method_post_paramc(csm_bc_method *m) {
    ...
    if (csm_bc_get_string(&argstr, m->parent, m->sigblock)) {
        return CSM_ERR_LOGICAL;
    }
...
static int object_post_fieldc(csm_bc_object *o)
{
    ...
    /* Error if marked as zero-args and sigblock index is not zero. */
    if (o->is_zero_field && o->fieldblock) {
        return CSM_ERR_LOGICAL;
    }
    ...
    /* Error if invalid string index. */
    if (csm_bc_get_string(&fieldstr, o->parent, o->fieldblock)) {
        return CSM_ERR_LOGICAL;
    }
    ...
    for (i = 0; i < fieldstr.length; i++) {
        ...
        /* Error if we hit two separators in a row. */
        if (is_last_sep && cursor == CSM_FIELD_SEP_CHAR) {
            return CSM_ERR_LOGICAL;
        }
        ...
        if (cursor == CSM_FIELD_SEP_CHAR) {
            is_last_sep = CSM_TRUE;
        }
    }

    /* Error if we've exceeded the max argument count. */
    if (fieldcount>= CSM_FIELD_COUNT_MAX) {
        return CSM_ERR_LOGICAL;
    }

    /* Write out param count as a side effect. */
    o->post_fieldc = fieldcount;
...
int csm_bc_object_postinit(csm_bc_object *o)
{
    ...
    ATTEMPT_AND_JUMP_ON_ERROR(object_post_fieldc, o, err, _end);
